...
public class RegisterAccount extends Account {
    ...
    private static final double initialDeposit = 500.0;
    
    public RegisterAccount(String accountNumber) {
        super(accountNumber);
    }
    ...
    private static void createAccountFile() {
        ...
        File file = new File(ACCOUNT_DETAILS_FILE);
        try {
            if (!file.exists()) {
                file.createNewFile();
            }
        } catch (IOException e) {
            System.out.println("Error creating account details file: " + e.getMessage());
        }
    }
